...
describe("seo", () => {
  it("should return default meta tags when no parameters are provided", () => {
    ...
    expect(tags).toContainEqual({ name: "keywords", content: metadata.keywords.join(", ") });
    ...
    expect(tags).toContainEqual({ name: "twitter:creator", content: metadata.twitter });
    expect(tags).toContainEqual({ name: "twitter:site", content: metadata.twitter });
    expect(tags).toContainEqual({ name: "og:type", content: "website" });
    expect(tags).toContainEqual({ name: "og:title", content: metadata.title });
    expect(tags).toContainEqual({ name: "og:description", content: metadata.description });
    
    // Should not include image tags when no image is provided
    expect(tags).not.toContainEqual({ name: "twitter:image", content: expect.any(String) });
    expect(tags).not.toContainEqual({ name: "twitter:card", content: expect.any(String) });
    expect(tags).not.toContainEqual({ name: "og:image", content: expect.any(String) });
  });
  ...
  it("should append custom title to base title", () => {
    ...
    expect(tags).toContainEqual({ name: "twitter:title", content: expectedTitle });
  ...
  it("should use custom description when provided", () => {
    ...
    expect(tags).toContainEqual({ name: "description", content: customDescription });
    expect(tags).toContainEqual({ name: "twitter:description", content: customDescription });
    expect(tags).toContainEqual({ name: "og:description", content: customDescription });
  });
  ...
  it("should include image meta tags when image is provided", () => {
    ...
    expect(tags).toContainEqual({ name: "twitter:image", content: customImage });
  ...
  it("should merge custom keywords with default keywords", () => {
    ...
    expect(tags).toContainEqual({ name: "keywords", content: expectedKeywords });
  });
  ...
  it("should handle all parameters together", () => {
    ...
    expect(tags).toContainEqual({ title: expectedTitle });
    expect(tags).toContainEqual({ name: "description", content: params.description });
    expect(tags).toContainEqual({ name: "keywords", content: expectedKeywords });

    // Check Twitter tags
    expect(tags).toContainEqual({ name: "twitter:title", content: expectedTitle });
    expect(tags).toContainEqual({ name: "twitter:description", content: params.description });
    expect(tags).toContainEqual({ name: "twitter:creator", content: metadata.twitter });
    expect(tags).toContainEqual({ name: "twitter:site", content: metadata.twitter });
    expect(tags).toContainEqual({ name: "twitter:image", content: params.image });
    expect(tags).toContainEqual({ name: "twitter:card", content: "summary_large_image" });

    // Check Open Graph tags
    expect(tags).toContainEqual({ name: "og:type", content: "website" });
    expect(tags).toContainEqual({ name: "og:title", content: expectedTitle });
    expect(tags).toContainEqual({ name: "og:description", content: params.description });
  ...
  it("should handle only description parameter", () => {
    ...
    expect(tags).toContainEqual({ name: "description", content: "Only Description" });
    expect(tags).toContainEqual({ name: "twitter:description", content: "Only Description" });
    expect(tags).toContainEqual({ name: "og:description", content: "Only Description" });
