...
import { metadata, seo } from "./metadata";
...
describe("metadata", () => {
  ...
  describe("seo", () => {
    ...
    it("should preserve special characters in content", () => {
      ...
      expect(tags).toContainEqual({ title: expectedTitle });
      expect(tags).toContainEqual({ name: "description", content: specialChars });
    });
    ...
    it("should use fallback correctly for image with falsy values", () => {
      ...
      const imageTagsUndefined = tagsWithUndefined.filter(tag =>
        tag.name === "twitter:image" ||
        tag.name === "twitter:card" ||
        tag.name === "og:image"
      );
      ...
      expect(tagsWithValue).toContainEqual({ name: "og:image", content: "/actual.png" });
    });
  });
});