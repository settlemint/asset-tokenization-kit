 ...
 */
export async function setPincode(user: UserWithPincodeContext, pincode: string) {
  if (!user.wallet) {
 ...
 */
export async function removePincode(user: UserWithPincodeContext, verificationId: string) {
  if (!user.wallet) {
    throw new APIError("BAD_REQUEST", {
      message: "User wallet not found",
    });
 ...
 */
export async function updatePincode(user: UserWithPincodeContext, newPincode: string) {
