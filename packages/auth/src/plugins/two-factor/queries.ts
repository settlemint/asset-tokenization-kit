...
export async function disableTwoFactor(user: UserWithTwoFactorContext) {
  ...
  const result = await portalClient.request(DELETE_WALLET_VERIFICATION_MUTATION, {
    address: user.wallet,
 ...
 */
export async function verifyTwoFactorOTP(user: UserWithTwoFactorContext, code: string) {
  if (!user.wallet) {
    throw new APIError("BAD_REQUEST", {
      message: "Wallet address is not set",
    });
  ...
  const result = await portalClient.request(VERIFY_WALLET_VERIFICATION_CHALLENGE_MUTATION, {
    address: user.wallet,
    verificationId: user.twoFactorVerificationId,
    otp: code,
  });
...
type OTPAlgorithm = VariablesOf<typeof CREATE_WALLET_VERIFICATION_MUTATION>["algorithm"];
...
export async function enableTwoFactor(
  { algorithm, digits, period }: { algorithm: OTPAlgorithm; digits: number; period: number },
  ...
  const result = await portalClient.request(CREATE_WALLET_VERIFICATION_MUTATION, {
    address: user.wallet,
    algorithm,
    digits,
    period,
  ...
  if (!result.createWalletVerification?.id) {
    throw new APIError("INTERNAL_SERVER_ERROR", {
      message: "Failed to create wallet verification, no verification ID returned",
    });
  }
