...
import { updateSession, validatePassword } from "../utils";
    ...
    endpoints: {
      generateSecretCodes: createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          const result = await portalClient.request(GENERATE_SECRET_CODES_MUTATION, {
            address: user.wallet,
          });
      ...
      confirmSecretCodes: createAuthEndpoint(
        "/secret-codes/confirm",
        {
          method: "POST",
          body: z.object({
            stored: z.boolean().describe("Whether the secret codes were stored"),
          }),
          use: [sessionMiddleware],
          metadata: {
            openapi: {
