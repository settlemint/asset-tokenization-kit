...
import { getTestOrpcClient } from "../../../../kit/dapp/test/fixtures/orpc-client.ts";
  ...
  const { error: signInError } = await authClient.signIn.email(
    ...
    },
    {
      onSuccess(context) {
        ...
        if (!cookieFound) {
          console.warn(`[signInWithUser] No session cookie found in response headers for ${user.email}`);
        }
      },
    }
  );
  ...
  try {
    ...
    if (!userWallet || userWallet === zeroAddress) {
      ...
      try {
        ...
        console.error(`[setupUser] Failed to create wallet for ${user.email}:`, {
    ...
    if (secretCodeError) {
      console.error(`[setupUser] Generate secret code error for ${user.email}:`, {
    ...
    if (confirmSecretCodeError) {
      console.error(`[setupUser] Confirm secret code error for ${user.email}:`, {
