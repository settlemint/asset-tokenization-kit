...
// The portal client should be mocked via the bun:test config alias
// If the mock isn't working, we'll manually set it up
if (typeof portalClient.request !== "function" || !(portalClient.request as any).mockResolvedValueOnce) {
  // Replace the request method with a mock
  (portalClient as unknown as { request: Mock }).request = jest.fn();
}
