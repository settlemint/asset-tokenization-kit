import type { AccessControl, AccessControlRoles } from "@atk/auth/fragments/the-graph/access-control-fragment";

/**
 * Type guard to check if a value is an array of objects with id and isContract properties
 */
function isAccountArray(value: unknown): value is Array<{ id: string; isContract: boolean }> {
  return (
    Array.isArray(value) &&
    value.every(
      (item) =>
...
export function hasRole(
  ...
  return roleAccounts.some((account) => account.id.toLowerCase() === wallet.toLowerCase());
}
 ...
 */
export function getUserRoles(wallet: string, accessControl: AccessControl | null | undefined): AccessControlRoles[] {
