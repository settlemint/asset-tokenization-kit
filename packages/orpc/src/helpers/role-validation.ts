 ...
 */
export function mapUserRoles(wallet: EthereumAddress, accessControl: AccessControl | null) {
  ...
  const userRoles = getAccessControlEntries(accessControl).reduce<Record<AccessControlRoles, boolean>>(
    (acc, [role, accounts]) => {
      const userHasRole = accounts.some((account) => account.id.toLowerCase() === wallet.toLowerCase());
