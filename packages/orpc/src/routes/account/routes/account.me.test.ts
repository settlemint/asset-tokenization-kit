...
describe("account.me (unit)", () => {
  it("throws UNAUTHORIZED when no auth context", async () => {
    ...
    await expect(
      handler({
        context: { theGraphClient: { query: jest.fn() } },
        input: undefined,
  ...
  it("returns account from nested read", async () => {
    ...
    const result = await handler({
      context: createBaseContext({ theGraphClient: { query: jest.fn() } }),
      input: undefined,
      errors: createMockErrors(),
    });
