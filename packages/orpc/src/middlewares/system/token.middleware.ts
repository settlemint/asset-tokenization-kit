 ...
 */
export const tokenMiddleware = baseRouter.middleware(async ({ next, context, errors }, input) => {
  ...
  if (!userClaimTopics) {
    logger.warn("userClaimsMiddleware should be called before tokenMiddleware");
    ...
    userPermissions: {
      ...
      actions: (() => {
        ...
        for (const [action, roleRequirement] of Object.entries(TOKEN_PERMISSIONS)) {
          ...
          initialActions[action as keyof typeof TOKEN_PERMISSIONS] = satisfiesRoleRequirement(
            userRoleList,
            roleRequirement
          );
        }
