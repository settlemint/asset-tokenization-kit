import { type AccessControl, AccessControlFragment } from "@atk/auth/fragments/the-graph/access-control-fragment";
 ...
 */
export const systemMiddleware = baseRouter.middleware(async ({ context, next, errors }) => {
...
export const getSystemContext = async (systemAddress: EthereumAddress): Promise<SystemContext | null> => {
  ...
  const complianceModules =
    system.complianceModuleRegistry?.complianceModules.map(({ id, typeId, name }) => ({
      id: getEthereumAddress(id),
