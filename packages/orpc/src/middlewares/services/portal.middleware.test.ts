...
// Import from the mocked module - bun:test config alias handles this
...
import { portalMiddleware, type ValidatedPortalClient } from "./portal.middleware";

// Note: This test runs in Node environment (@bun:test-environment node)
// to avoid browser environment checks in the SDK

// Note: portalClient is mocked via bun:test config alias
...
describe("portal.middleware", () => {
  ...
  let mockErrors: Record<string, ReturnType<typeof jest.fn>>;
  ...
  test("browser environment should be properly mocked", () => {
    ...
    expect((globalThis as any).window?.document).toBeUndefined();
  ...
  beforeEach(() => {
    ...
    (portalClient.request as any) = jest.fn();

    // Create mock errors object
    mockErrors = {
      PORTAL_ERROR: jest.fn((params) => {
  ...
  describe("middleware initialization", () => {
    ...
    test("should reuse existing client when context already has mutate method", async () => {
      const existingClient = { mutate: jest.fn(), query: jest.fn() };
  ...
  describe("mutation execution", () => {
    ...
    describe("transaction hash extraction", () => {
      test("should extract transaction hash from direct response", async () => {
        ...
        expect(portalClient.request).toHaveBeenCalledTimes(1);
