...
function generatePincodeResponse(pincode: string, salt: string, secret: string): string {
  ...
  const hashedPincode = createHash("sha256").update(`${salt}${pincode}`).digest("hex");
  // PHASE 2: Combine with server secret to prevent replay attacks
  return createHash("sha256").update(`${hashedPincode}_${secret}`).digest("hex");
}
    ...
    async mutate<TResult, TVariables extends Variables & { from: string }>(
      ...
      if (walletVerification) {
        ...
        if (type === "OTP") {
          ...
          challengeResponse = generatePincodeResponse(code, challenge.challenge.salt, challenge.challenge.secret);
        }
      ...
      try {
        ...
        throw errors.PORTAL_ERROR({
          message: mapPortalErrorMessage(errorMessage) ?? `GraphQL ${operation} failed`,
          data: {
            document,
            variables,
          },
      ...
      for (let attempt = 0; attempt < Math.ceil(INDEXING_TIMEOUT_MS / POLLING_INTERVAL_MS); attempt++) {
        if (Date.now() - streamStartTime > STREAM_TIMEOUT_MS) {
          throw errors.PORTAL_ERROR({
 ...
 * ```
 */
function findTransactionHash(obj: unknown, path = ""): { value: unknown; path: string } | null {
...
export const portalMiddleware = baseRouter.middleware((options) => {
  ...
  const portalClient = createValidatedPortalClient(errors, context.theGraphClient);
...
export type ValidatedPortalClient = ReturnType<typeof createValidatedPortalClient>;
