 ...
 * });
 */
function createValidatedTheGraphClient(errors: Parameters<Parameters<typeof baseRouter.middleware>[0]>[0]["errors"]) {
  return {
    async query<TResult, TVariables extends Variables, TValidated>(
      document: TadaDocumentNode<TResult, TVariables>,
      options: {
        input: Omit<TVariables, "skip" | "first" | "orderBy" | "orderDirection">;
      ...
      try {
        rawResult = await theGraphClient.request(document as RequestDocument, input as Variables);
        return schema.parse(rawResult);
      } catch (error) {
        throw errors.THE_GRAPH_ERROR({
          message:
            error instanceof ClientError
              ? (error.response.errors?.map((e) => e.message).join(", ") ?? (error as Error).message)
              : (error as Error).message,
          data: {
            document,
            variables: input as TVariables,
            responseValidation: error instanceof z.ZodError ? z.prettifyError(error) : undefined,
          },
          cause: error,
        });
      }
...
export type ValidatedTheGraphClient = ReturnType<typeof createValidatedTheGraphClient>;
