...
describe("User search", () => {
  ...
  beforeAll(async () => {
    // Setup test users
    await Promise.all([setupUser(TEST_USER), setupUser(OTHER_USER), setupUser(UNAUTHORIZED_USER)]);
    ...
    const testUserClient = getTestOrpcClient(testUserHeaders);
    const otherUserClient = getTestOrpcClient(otherUserHeaders);
  ...
  describe("Admin access", () => {
    ...
    it("admin can search users by partial name match", async () => {
      ...
      const foundUsers = users.filter((u) => u.id === testUserData.id || u.id === otherUserData.id);
