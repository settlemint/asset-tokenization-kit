...
describe("the-graph.middleware", () => {
  ...
  test("browser environment should be properly mocked", () => {
    ...
    const windowWithDoc = globalThis as Record<string, unknown> & {
      window?: { document?: unknown };
    };
    expect(windowWithDoc.window?.document).toBeUndefined();
  ...
  beforeEach(() => {
    ...
    const mockRequest = theGraphClient.request as ReturnType<typeof jest.fn>;
    if (mockRequest.mockReset) {
      mockRequest.mockReset();
    }
    (theGraphClient.request as jest.Mock) = jest.fn();

    // Create mock errors object
    mockErrors = {
      THE_GRAPH_ERROR: jest.fn((params) => {
  ...
  describe("middleware initialization", () => {
    ...
    test("should reuse existing client when context already has one", async () => {
      const existingClient = { query: jest.fn() };
