...
async function analyzePRAndApplyLabels({
  ...
  if (prTitle.match(/^(chore|fix|build)\(deps\):/)) {
    ...
    const typeMatch = prTitle.match(/^(feat|fix|docs|style|refactor|perf|test|build|ci|revert)(\(.+\))?:/);
    if (typeMatch) {
      ...
      console.log('No conventional commit format detected, defaulting to chore');
    }
  }
  
  // Check for breaking changes
  if (prTitle.match(/^(feat|fix|docs|style|refactor|perf|test|build|ci|revert)(\(.+\))?!:/) || 
