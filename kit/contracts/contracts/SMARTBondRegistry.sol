...
/// @title SMARTBondRegistry - Tracks registered SMARTBond contracts
/// @notice This contract maintains a registry of SMARTBond token contracts.
/// Only the owner can register or unregister bond contracts.
/// @dev Inherits from Ownable and ERC2771Context for ownership management and meta-transaction support.
/// @custom:security-contact support@settlemint.com
contract SMARTBondRegistry is ERC2771Context, Ownable {
    ...
    /// @notice Mapping indicating whether a bond token address is registered.
    mapping(address => bool) public isTokenRegistered;

    /// @notice Emitted when a bond token is registered.
    /// @param token The address of the registered bond token.
    event TokenRegistered(address indexed token);

    /// @notice Emitted when a bond token is unregistered.
    /// @param token The address of the unregistered bond token.
    event TokenUnregistered(address indexed token);

    /// @notice Deploys the registry contract.
    /// @dev Sets the initial owner who can register and unregister bonds.
    /// @param forwarder The address of the trusted forwarder for meta-transactions.
    /// @param initialOwner The address of the initial owner.
    constructor(address forwarder, address initialOwner) Ownable(initialOwner) ERC2771Context(forwarder) { }

    /// @notice Registers a SMARTBond contract.
    /// @dev Can only be called by the owner.
    /// Reverts if the token address is invalid or if the token is already registered.
    /// @param token The address of the SMARTBond contract to register.
    ...
    /// @notice Unregisters a SMARTBond contract.
    /// @dev Can only be called by the owner.
    /// Reverts if the token address is invalid or if the token is not registered.
    /// @param token The address of the SMARTBond contract to unregister.
