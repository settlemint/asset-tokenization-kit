...
import { AccessDenied, EMPTY_UID, InvalidLength, NotFound, NO_EXPIRATION_TIME } from "@eas/contracts/Common.sol";
import {
    Attestation,
    AttestationRequest,
...
contract EAS is IEAS, Semver, EIP1271Verifier, ERC2771Context {
    ...
    function attest(AttestationRequest calldata request) external payable returns (bytes32) {
        ...
        return _attest(request.schema, data, _msgSender(), msg.value, true).uids[0];
    }
        ...
        returns (bytes32[] memory)
    {
        ...
        for (uint256 i = 0; i < length; ++i) {
            ...
            AttestationsResult memory res =
                _attest(multiRequest.schema, multiRequest.data, _msgSender(), availableValue, last);
    ...
    function revoke(RevocationRequest calldata request) external payable {
        ...
        _revoke(request.schema, data, _msgSender(), msg.value, true);
    }
    ...
    function multiRevoke(MultiRevocationRequest[] calldata multiRequests) external payable {
        ...
        for (uint256 i = 0; i < length; ++i) {
            ...
            // Ensure to deduct the ETH that was forwarded to the resolver during the processing of this batch.
            availableValue -= _revoke(multiRequest.schema, multiRequest.data, _msgSender(), availableValue, last);
        }
    }
    ...
    function revokeOffchain(bytes32 data) external returns (uint64) {
        ...
        _revokeOffchain(_msgSender(), data, time);
    ...
    function multiRevokeOffchain(bytes32[] calldata data) external returns (uint64) {
        ...
        for (uint256 i = 0; i < length; ++i) {
            _revokeOffchain(_msgSender(), data[i], time);
        }
    ...
    function _refund(uint256 remainingValue) private {
        if (remainingValue > 0) {
            ...
            // apply for.
            payable(_msgSender()).sendValue(remainingValue);
        }
    }
