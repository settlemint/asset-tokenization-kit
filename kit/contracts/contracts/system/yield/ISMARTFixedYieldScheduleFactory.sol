...
interface ISMARTFixedYieldScheduleFactory {
    ...
    /// @param schedule The address of the newly deployed `SMARTFixedYieldProxy` contract.
    ...
    /// named `smartFixedYieldScheduleImplementation`.
    function smartFixedYieldScheduleImplementation() external view returns (address);

    /// @notice Creates a new SMARTFixedYieldSchedule proxy contract.
    /// @dev This function is expected to be available on the factory contract.
    /// It's typically created automatically if the factory has a public state variable
    /// named `smartFixedYieldScheduleImplementation`.
    function create(
