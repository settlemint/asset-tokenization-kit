...
library ATKSystemRoles {
    ...
    /// stuff directly there. Enable token factories and adding
    ...
    // --- DEPRECATED ROLES (for backward compatibility during migration) ---
    // These roles are deprecated and will be removed in future versions.
    // Use the new *_MANAGER_ROLE and *_MODULE_ROLE patterns instead.

    /// @notice Role for managing registration operations (DEPRECATED: Use IDENTITY_MANAGER_ROLE instead)
    /// @dev Assigned to entities responsible for user and contract registration (e.g., token factories)
    bytes32 public constant REGISTRAR_ROLE = keccak256("REGISTRAR_ROLE");

    /// @notice Role for managing registrar role assignments (DEPRECATED: Use IDENTITY_MANAGER_ROLE instead)
    ...
    /// @notice Role for managing claims (DEPRECATED: Use CLAIM_POLICY_MANAGER_ROLE instead)
    ...
    /// @notice Role for token deployers (DEPRECATED: Use TOKEN_MANAGER_ROLE instead)
    /// @dev Assigned to entities responsible for deploying new tokens
    bytes32 public constant DEPLOYER_ROLE = keccak256("DEPLOYER_ROLE");

    /// @notice Role for managing implementation addresses (DEPRECATED: Use SYSTEM_MANAGER_ROLE instead)
    ...
    /// @notice Role for modifying storage data (DEPRECATED: Use IDENTITY_REGISTRY_MODULE_ROLE instead)
    ...
    /// @notice Role for managing identity registry bindings (DEPRECATED: Use SYSTEM_MANAGER_ROLE instead)
    ...
    /// @notice Role for managing compliance bypass list (DEPRECATED: Use COMPLIANCE_MANAGER_ROLE instead)
    ...
    /// @notice Role for managing compliance bypass list admin (DEPRECATED: Use COMPLIANCE_MANAGER_ROLE instead)
    /// @dev This role allows adding/removing addresses from the bypass list that can bypass compliance checks
