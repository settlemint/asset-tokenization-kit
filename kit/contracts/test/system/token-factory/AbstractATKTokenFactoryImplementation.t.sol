...
contract AbstractATKTokenFactoryImplementationTest is Test {
	...
	function setUp() public {
		...
		bytes memory proxyCreationCode = abi.encodePacked(
			...
			abi.encode(address(factoryImpl), initData)
		);
	...
	function test_DeployContractIdentity_IssuesIssuerClaim() public {
		...
		(uint256 topic, , address issuer, , bytes memory data, )
			= IERC735(contractIdentity).getClaim(claimId);
	...
	function test_DeployContractIdentity_NoIssuerIdentity_SkipsIssuerClaim() public {
		...
		TestableTokenFactoryWithMockSystem factoryWithMockImpl = new TestableTokenFactoryWithMockSystem(
			forwarder,
			atkSystem.identityFactory(),
			atkSystem.identityRegistry()
		);
		...
		address contractIdentity = factoryWithMock.exposedDeployContractIdentity(
			mockContract,
			"Test Contract",
			840
		);
...
contract TestableTokenFactoryWithSystem is AbstractATKTokenFactoryImplementation {
	...
	function exposedCalculateSalt(string memory name, string memory symbol, uint8 decimals) external view returns (bytes32) {
