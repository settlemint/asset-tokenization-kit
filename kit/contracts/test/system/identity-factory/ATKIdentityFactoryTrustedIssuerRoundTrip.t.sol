...
contract ATKIdentityFactoryTrustedIssuerRoundTripTest is Test {
    ...
    /**
     * @notice Tests that wallets with other key purposes (not CLAIM_SIGNER, MANAGEMENT, or ACTION) cannot add
     * claims
     */
    function testWalletWithWrongKeyPurposeCannotAddClaims() public {
        ...
        vm.expectRevert(); // Should revert because ENCRYPTION_KEY (purpose 4) is not authorized for claim
            // operations
        userIdentity.addClaim(kycTopicId, 1, issuerIdentityAddr, signature, claimData, "");
    }
