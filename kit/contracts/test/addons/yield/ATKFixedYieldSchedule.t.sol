...
contract ATKFixedYieldScheduleTest is Test {
    ...
    function test_EstimateTotalYieldForCurrentPeriod() public {
        uint256 totalYield = yieldSchedule.estimateTotalYieldForCurrentPeriod();
        assertTrue(totalYield > 0);

        // After end date should be 0
        vm.warp(endDate + 1);
        assertEq(yieldSchedule.estimateTotalYieldForCurrentPeriod(), 0);
    }
