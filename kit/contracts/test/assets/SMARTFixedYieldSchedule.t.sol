...
contract SMARTFixedYieldScheduleTest is Test {
    ...
    function setUp() public {
        ...
        yieldSchedule =
            new SMARTFixedYieldSchedule(address(smartToken), startDate, endDate, RATE, INTERVAL, owner, forwarder, true);
    ...
    function test_InvalidConstructorParameters() public {
        ...
        new SMARTFixedYieldSchedule(
            address(smartToken), block.timestamp - 1, endDate, RATE, INTERVAL, owner, forwarder, true
        );

        // Invalid end date (before start)
        vm.expectRevert(ISMARTFixedYieldSchedule.InvalidEndDate.selector);
        new SMARTFixedYieldSchedule(
            address(smartToken), startDate, startDate - 1, RATE, INTERVAL, owner, forwarder, true
        );

        // Invalid rate (zero)
        vm.expectRevert(ISMARTFixedYieldSchedule.InvalidRate.selector);
        new SMARTFixedYieldSchedule(address(smartToken), startDate, endDate, 0, INTERVAL, owner, forwarder, true);

        // Invalid interval (zero)
        vm.expectRevert(ISMARTFixedYieldSchedule.InvalidInterval.selector);
        new SMARTFixedYieldSchedule(address(smartToken), startDate, endDate, RATE, 0, owner, forwarder, true);
    }
