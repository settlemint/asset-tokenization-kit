...
import { type ISIN, isin } from './isin';

const ALPHANUMERIC_9_REGEX = /^[A-Z0-9]{9}$/;
const NUMERIC_1_REGEX = /^[0-9]$/;
...
describe('isin', () => {
  ...
  describe('ISIN structure', () => {
    it('should follow correct structure', () => {
      ...
      expect(result.substring(2, 11)).toMatch(ALPHANUMERIC_9_REGEX);

      // Last char is numeric check digit
      expect(result.substring(11, 12)).toMatch(NUMERIC_1_REGEX);
    });
  });
