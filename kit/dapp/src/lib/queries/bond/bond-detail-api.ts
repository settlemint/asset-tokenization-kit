...
import { getBondDetail } from "./bond-detail";

export const BondDetailResponseSchema = t.Object({
  id: t.String({
    description: "The unique identifier (address) of the bond contract",
  }),
  name: t.String({
    description: "The name of the bond token",
  }),
  symbol: t.String({
    description: "The symbol/ticker of the bond token",
  }),
  decimals: t.Number({
  ...
  paused: t.Boolean({
    description: "Whether the bond contract is currently paused",
  }),
  creator: t.Object({
    id: t.String({
      description: "The address of the account that created the bond",
    }),
  }),
  holders: t.Array(
    t.Object({
  ...
  value_in_base_currency: t.Optional(
    t.Number({
      description: "The price of the bond token",
    })
  ),
});

export const BondDetailApi = new Elysia()
  .use(superJson)
  .use(betterAuth)
  .get(
    "/:address",
    ({ params: { address } }) => {
      return getBondDetail({ address: address as Address });
    },
    {
      auth: true,
      detail: {
