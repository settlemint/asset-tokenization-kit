import { defaultErrorSchema } from "@/lib/api/default-error-schema";
import { betterAuth, superJson } from "@/lib/utils/elysia";
import { Elysia, t } from "elysia";
import type { Address } from "viem";
import { getCryptoCurrencyDetail } from "./cryptocurrency-detail";

export const CryptoCurrencyDetailResponseSchema = t.Object({
  id: t.String({
    description:
      "The unique identifier (address) of the cryptocurrency contract",
  }),
  name: t.String({
    description: "The name of the cryptocurrency token",
  }),
  symbol: t.String({
    description: "The symbol/ticker of the cryptocurrency token",
  }),
  decimals: t.Number({
    description: "The number of decimal places used by the token",
  }),
  totalSupply: t.Number({
    description:
      "The total supply of tokens in circulation (formatted with decimals)",
  }),
  totalSupplyExact: t.BigInt({
    description: "The exact total supply of tokens in circulation (raw value)",
  }),
  totalHolders: t.Number({
    description: "The total number of unique addresses holding the token",
  }),
  creator: t.Object({
    id: t.String({
      description: "The address of the account that created the cryptocurrency",
    }),
  }),
  holders: t.Array(
    t.Object({
      valueExact: t.BigInt({
        description: "The exact token balance of the holder (raw value)",
      }),
    })
  ),
  isin: t.Optional(
    t.Union([
      t.Null(),
      t.String({
        description:
          "The International Securities Identification Number (ISIN) if assigned",
      }),
    ])
  ),
  concentration: t.Number({
    description: "The concentration of holdings among top holders",
  }),
  value_in_base_currency: t.Optional(
    t.Number({
      description: "The price of the cryptocurrency token",
    })
  ),
});

export const CryptoCurrencyDetailApi = new Elysia()
  .use(superJson)
  .use(betterAuth)
  .get(
    "/:address",
    ({ params: { address } }) => {
      return getCryptoCurrencyDetail({ address: address as Address });
    },
    {
      auth: true,
      detail: {
        summary: "Cryptocurrency Detail",
        description:
          "Retrieves a cryptocurrency token by address with details including supply and holder information.",
        tags: ["Cryptocurrencies"],
      },
      params: t.Object({
        address: t.String({
          description: "The address of the cryptocurrency token",
        }),
      }),
      response: {
        200: CryptoCurrencyDetailResponseSchema,
        ...defaultErrorSchema,
      },
    }
  );
