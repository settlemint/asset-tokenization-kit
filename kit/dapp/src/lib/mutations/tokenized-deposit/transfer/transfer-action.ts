...
import { TransferTokenizedDepositSchema } from './transfer-schema';
...
export const transfer = action
  .schema(TransferTokenizedDepositSchema)
  .outputSchema(z.hashes())
  .action(
    async ({ parsedInput: { address, pincode, value, to }, ctx: { user } }) => {
      const { decimals } = await getTokenizedDepositDetail({ address });

      const response = await portalClient.request(TokenizedDepositTransfer, {
        address,
        from: user.wallet,
