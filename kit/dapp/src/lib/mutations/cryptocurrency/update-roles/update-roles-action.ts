...
/**
 * GraphQL mutation to revoke a role from an account for a cryptocurrency
 *
 * @remarks
 * This mutation requires authentication via challenge response
 */
const CryptoCurrencyRevokeRole = portalGraphql(`
  mutation CryptoCurrencyRevokeRole($address: String!, $from: String!, $challengeResponse: String!, $role: String!, $account: String!) {
...
/**
 * Server action for updating a user's roles for a cryptocurrency
 *
 * @remarks
    ...
    async ({
      ...
      for (const [roleName, shouldHave] of Object.entries(roles)) {
        ...
        if (shouldHave) {
          ...
          if (response.CryptoCurrencyGrantRole?.transactionHash) {
            transactions.push(response.CryptoCurrencyGrantRole.transactionHash);
          }
        } else {
          ...
          if (response.CryptoCurrencyRevokeRole?.transactionHash) {
            transactions.push(
