...
/**
 * Function to revoke roles from a user for a cryptocurrency
 *
...
export async function revokeRole({
  ...
  for (const role of selectedRoles) {
    ...
    if (response.CryptoCurrencyRevokeRole?.transactionHash) {
      transactions.push(response.CryptoCurrencyRevokeRole?.transactionHash);
    }
  }
...
/**
 * React Query hook for revoking roles from a user for a cryptocurrency
 *
...
export function useRevokeRole() {
  const mutation = useMutation({
    mutationKey: ['cryptocurrency', 'revoke-role'],
    mutationFn: async ({
      pincode,
      from,
      address,
