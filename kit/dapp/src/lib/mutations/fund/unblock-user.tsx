...
/**
 * GraphQL mutation for unblocking a user from using a fund
 *
 * @remarks
 * Restores a previously blocked user's ability to interact with the fund
 */
const FundUnblockUser = portalGraphql(`
  mutation FundUnblockUser($address: String!, $challengeResponse: String!, $from: String!, $user: String!) {
    FundUnblockUser(
      from: $from
      input: {user: $user}
      address: $address
...
/**
 * React Query hook for unblocking a user from using a fund
 *
...
export function useUnblockUser() {
  const mutation = useMutation({
    ...
    mutationFn: async ({
      ...
      const response = await portalClient.request(FundUnblockUser, {
        address: address,
        from,
        user: userAddress,
      ...
      return z.hash().parse(response.FundUnblockUser?.transactionHash);
    },
  });
