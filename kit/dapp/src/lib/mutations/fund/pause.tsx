 ...
 *
 * @remarks
 * Temporarily suspends all transfers and operations on the fund
 */
...
/**
 * React Query hook for pausing a fund contract
 *
 ...
 *   } catch (error) {
 *     toast.error("Failed to pause fund");
 *   }
 * };
 * ```
 */
export function usePause() {
  const mutation = useMutation({
    ...
    mutationFn: async ({ pincode, from, address }: Pause) => {
      ...
      return z.hash().parse(response.FundPause?.transactionHash);
    },
  });
