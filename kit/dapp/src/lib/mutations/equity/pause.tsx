...
/**
 * React Query hook for pausing a equity contract
 *
 ...
 *       from: "0x789..."
 *     });
 *     toast.success("Equity paused successfully");
 *   } catch (error) {
 *     toast.error("Failed to pause equity");
 *   }
 * };
 * ```
 */
export function usePause() {
  const mutation = useMutation({
    mutationKey: ['equity', 'pause'],
    mutationFn: async ({ pincode, from, address }: Pause) => {
      const response = await portalClient.request(EquityPause, {
        address: address,
        from,
        challengeResponse: await handleChallenge(from, pincode),
      });

      return z.hash().parse(response.EquityPause?.transactionHash);
    },
  });
