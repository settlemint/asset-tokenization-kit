...
export function normalizeDecimalInput(value: string, locale: string): string {
  ...
  if (thousandsSeparator) {
    ...
    const escaped = thousandsSeparator.replaceAll(
      /[.*+?^${}()|[\]\\]/g,
      String.raw`\$&`
    );
  ...
  if (decimalSeparator !== ".") {
    const escaped = decimalSeparator.replaceAll(
      /[.*+?^${}()|[\]\\]/g,
      String.raw`\$&`
    );
    normalized = normalized.replaceAll(new RegExp(escaped, "g"), ".");
  ...
  // Remove trailing decimal point to make valid for dnum.from()
  // "1234." becomes "1234" which is parseable
