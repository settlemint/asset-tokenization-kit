...
export function getErrorCode(error: unknown): string | number {
  ...
  if (errorObj?.status) {
    return errorObj.status as string | number;
  }
  if (errorObj?.statusCode) {
    return errorObj.statusCode as string | number;
  }
  if (errorObj?.code) {
    return errorObj.code as string | number;
  }
  if (errorObj?.response && typeof errorObj.response === 'object') {
    const response = errorObj.response as Record<string, unknown>;
    if (response.status) {
      return response.status as string | number;
    }
    if (response.statusCode) {
      return response.statusCode as string | number;
    }
  }
