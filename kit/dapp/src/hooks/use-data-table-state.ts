...
export function useDataTableState(
  ...
  const rawUrlSearchParams = useSearch({ strict: false, from: routePath as undefined });
  ...
  const initialState = useMemo(() => {
    if (enableUrlPersistence && Object.keys(urlSearchParams).length > 0) {
      ...
      return {
        ...
        sorting: deserializedState.sorting.length > 0 ? deserializedState.sorting : initialSorting,
        columnFilters: deserializedState.columnFilters.length > 0 ? deserializedState.columnFilters : initialColumnFilters,
        globalFilter: deserializedState.globalFilter || "",
        columnVisibility: Object.keys(deserializedState.columnVisibility).length > 0 ? deserializedState.columnVisibility : initialColumnVisibility,
  ...
  const [columnFilters, setColumnFiltersState] = useState<ColumnFiltersState>(initialState.columnFilters);
  const [globalFilter, setGlobalFilterState] = useState<string>(initialState.globalFilter);
  const [columnVisibility, setColumnVisibilityState] = useState<VisibilityState>(initialState.columnVisibility);
  const [rowSelection, setRowSelectionState] = useState<RowSelectionState>(initialState.rowSelection);
  ...
  const updateUrl = useCallback((newState: Partial<DataTableSearchParams>) => {
  ...
  const setPagination = useCallback((updater: PaginationState | ((old: PaginationState) => PaginationState)) => {
  ...
  const setGlobalFilter = useCallback((updater: string | ((old: string) => string)) => {
  ...
  useEffect(() => {
    if (enableUrlPersistence && Object.keys(urlSearchParams).length > 0) {
      ...
      setPaginationState(tableState.pagination ?? { pageIndex: 0, pageSize: defaultPageSize });
