...
export async function GET(request: Request, { params }: RouteParams) {
  ...
  const { cryptoCurrency } = await theGraphClientStarterkits.request(OgCryptoCurrency, {
    id: fundAddress,
  });
    ...
    <div tw="flex flex-col w-full h-full">
      ...
      <div tw="flex flex-col absolute top-48 left-32 text-white">
        ...
        <p tw="text-xl mt-0 flex items-center">
          {/* biome-ignore lint/nursery/noImgElement: <explanation> */}
          <img src={avatarUrl} alt={cryptoCurrency.name} tw="w-8 h-8 rounded-full mr-2" />
        ...
        <div tw="flex gap-24 mt-16 pr-32">
          <div tw="flex flex-col p-8 rounded-2xl border-4 border-white/30 bg-white/5">
            ...
            <dd tw="text-4xl font-bold text-white mt-2">
              {formatTokenValue(BigInt(cryptoCurrency.totalSupply), { decimals: 2 })}
            </dd>
          </div>
        </div>
      </div>
