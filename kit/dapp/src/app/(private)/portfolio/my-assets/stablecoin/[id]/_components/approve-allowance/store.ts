...
export const approveStablecoin = actionClient
  ...
  .action(async ({ parsedInput: { address, to, amount, pincode, decimals }, ctx: { user } }) => {
    const data = await portalClient.request(StableCoinApprove, {
      address: address,
      from: user.wallet,
      to: to,
    ...
    const transactionHash = data.StableCoinApprove?.transactionHash;
    if (!transactionHash) {
