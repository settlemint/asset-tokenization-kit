...
function PlatformOnboarding() {
  ...
  const getInitialStep = useCallback(() => {
    ...
    if (!session?.user?.pincodeEnabled) {
      return 'security';
    }
    if (!systemAddress) {
      return 'system';
  ...
  }, [user, hasInitialized, session?.user, getInitialStep]);
  ...
  return (
    <OnboardingGuard allowedTypes={['platform']} require="not-onboarded">
