...
const tokenPermissionsSelections = [
...
export const TokenPermissionsListInput = ({
  ...
  const _tokenPermissions = useWatch({ control: form?.control, name: 'tokenPermissions' });
  const [listItems, setListItems] = useState<Pick<TokenUser, 'id' | 'wallet' | 'email' | 'name'>[]>(_tokenPermissions);

  // biome-ignore lint/correctness/useExhaustiveDependencies: <explanation>
  useEffect(() => {
    console.log('_tokenPermissions', _tokenPermissions);
    console.log('listItems', listItems);
    form?.trigger('tokenPermissions');
  }, [_tokenPermissions, form, listItems.length]);
        ...
        {listItems.map((user, index) => {
          return (
            <li className="ListItem my-6" key={user.wallet?.toString() + index}>
              ...
              <MultiSelectInput
                entries={tokenPermissionsSelections}
