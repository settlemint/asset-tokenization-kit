...
import "./trusted-issuer.create";
import type {
  TrustedIssuerCreateInput,
  TrustedIssuerCreateOutput,
...
describe("system.trusted-issuers.create unit", () => {
  ...
  it("creates trusted issuer with valid input and permissions", async () => {
    ...
    const input: TrustedIssuerCreateInput = {
    ...
    expect(context.portalClient.mutate).toHaveBeenCalledWith(
      expect.stringContaining("CreateTrustedIssuerMutation"), // The GraphQL mutation
      {
  ...
  it("creates trusted issuer with single claim topic", async () => {
    const handler = getHandler();
    const context = createBaseContext({
      system: {
        systemAccessManager: {
    ...
    expect(context.portalClient.mutate).toHaveBeenCalledWith(
      expect.stringContaining("CreateTrustedIssuerMutation"),
      {
        address: "0xCCCCcCCcCcCcCcCcCcCcCcCcCcCcCcCcCcCcCcCc",
        from: context.auth.user.wallet,
