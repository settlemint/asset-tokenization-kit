...
const MAX_PRELOAD_BYTES = Number(
  process.env.STATIC_PRELOAD_MAX_BYTES ?? 10 * 1024 * 1024 // 10MB default
);

// Parse comma-separated include patterns
const INCLUDE_PATTERNS = (
  process.env.STATIC_PRELOAD_INCLUDE ??
  "*.js,*.css,*.woff2,*.png,*.svg,*.webmanifest,*.ico"
)
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean)
...
const EXCLUDE_PATTERNS = (process.env.STATIC_PRELOAD_EXCLUDE ?? "*.map,*.txt")
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean)
  .map(globToRegExp);
...
const VERBOSE = process.env.STATIC_PRELOAD_VERBOSE !== "false";
