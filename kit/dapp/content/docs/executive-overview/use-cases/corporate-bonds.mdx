...
fees, automate quarterly coupon distributions, and gain real-time cap table
visibility.
...
- **Automated yield** for quarterly coupon distributions without manual
  processing
...
automate quarterly coupon payments** and connects the bond to the XvP settlement
system for atomic trading.
...
### Automated coupon payments via yield schedules

**ATK's fixed yield schedule contract automates quarterly distributions without
manual processing.** Every quarter, the yield system calculates interest owed to
each holder based on proportional ownership at the snapshot block. Token holders
claim their coupons directly through the smart contract with cryptographic proof
of entitlement.

The platform triggers distributions through:

- **Tokenized cash settlement** – Automated distributions when investors hold
...
yield schedule is configured once during issuance; distributions happen
automatically on payment dates. No spreadsheets. No transfer agent. No manual
wire transfers.
...
| **Coupon processing**      | Manual calculation & wire instructions | Automated scheduler with payment adapters |
...
- Explore [Developer Documentation](/docs/developer-guides) for technical
  integration details
