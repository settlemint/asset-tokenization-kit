...
MidCorp allocates tokens to a subscriber list. The platform enforces that every
recipient meets eligibility requirements, eliminating risk of non-compliant
distribution.

### Automated coupon payments

Every quarter, the coupon scheduler calculates interest owed to each holder
based on proportional ownership. The platform triggers distributions through:

- **Tokenized cash settlement** – If investors hold stablecoins or tokenized
  deposits
- **Off-chain payment adapters** – Integration with treasury systems for
  traditional wire/ACH settlement
- **Manual approval workflows** – For jurisdictions requiring human review
  before disbursement

The coupon module ensures every holder receives their exact amount without
manual calculations or wire transfer instructions. Note that fully automated
end-to-end settlement requires either tokenized payment instruments or
configured off-chain adapters.
