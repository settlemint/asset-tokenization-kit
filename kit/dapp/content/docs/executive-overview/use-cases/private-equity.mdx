...
performance visibility, calculate distribution entitlements with on-demand
claiming, and facilitate compliant secondary transfers.
...
When portfolio companies exit or pay dividends, Apex calculates entitlements
proportionally for all token holders. Token holders claim their proceeds
on-demand through the smart contract. What used to require wire instructions to
80 different banks now executes through claim-based distribution with instant
settlement (via tokenized cash) or configured payment adapters (for fiat
settlement).
...
| **Distribution processing** | Wire to 80 banks, days of work      | Calculated entitlements, on-demand claiming |
...
**LP satisfaction** – Real-time holdings visibility and on-demand distribution
claiming improve investor experience. Support inquiries about "where's my
