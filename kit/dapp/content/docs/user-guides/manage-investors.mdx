...
claim issuance, and ongoing compliance monitoring. This guide walks compliance
officers through the complete investor lifecycle from onboarding through ongoing
surveillance.

**Who should read this:** Compliance officers, KYC analysts, investor relations
teams, and transfer agents managing participant eligibility and regulatory
compliance.
...
3. Choose invitation method:
   ...
   - **Bulk import** - Upload CSV of multiple investors

![Investor invitation](/images/screenshots/placeholder.svg) _Screenshot:
Investor invitation modal showing email input, invitation method options, and
bulk upload interface_
...
![KYC form](/images/screenshots/placeholder.svg) _Screenshot: KYC data
collection form showing personal information fields, document upload areas, and
investor type selection_
...
![KYC review interface](/images/screenshots/placeholder.svg) _Screenshot:
Compliance officer KYC review showing investor documents, verification
checklist, sanctions screening results, and approve/reject buttons_
...
![OnchainID deployment](/images/screenshots/placeholder.svg) _Screenshot:
Investor profile showing KYC status verified, OnchainID contract deployment
progress, and resulting OnchainID address_
...
3. Click **Issue Claim**
4. Select claim type from dropdown (e.g., ACCREDITED_INVESTOR)
5. Enter claim data:
   - **Claim topic** - Automatically filled based on type
   - **Claim data** - Encoded investor information (auto-generated)
   - **Expiration date** - When claim expires (optional, e.g., 1 year for
     accredited status)
...
![Claim issuance](/images/screenshots/placeholder.svg) _Screenshot: Claim
issuance form showing claim type dropdown, automatically generated claim data,
expiration date picker, and issue button_
...
### Investor eligibility dashboard

Track which investors can hold which assets:

1. Navigate to **Compliance** &gt; **Eligibility Matrix**
2. View table showing:
   - Rows: All registered investors
   - Columns: All deployed tokens
   - Cells: Green (eligible), Red (ineligible), Yellow (pending)
3. Click cell to see eligibility details:
   - Required claims for token
   - Investor's current claims
   - Missing claims preventing eligibility
   - Expired claims needing renewal

![Eligibility matrix](/images/screenshots/placeholder.svg) _Screenshot:
Eligibility matrix showing grid of investors vs tokens with color-coded
eligibility status_

**Proactive monitoring:** Use matrix to identify investors losing eligibility
due to expiring claims before they attempt transactions.
...
1. Review expiring claims report: **Compliance** &gt; **Expiring Claims**
...
If investor fails ongoing monitoring (e.g., added to sanctions list):

1. Navigate to investor profile
2. Click **Actions** &gt; **Revoke Claim**
3. Select claim to revoke (e.g., KYC_VERIFIED)
...
1. Navigate to **Compliance** &gt; **Monitoring Settings**
2. Enable **Continuous Screening**
3. Configure screening frequency:
   - Daily screening against sanctions lists
   - Weekly PEP list updates
   - Monthly adverse media searches
4. Set alert thresholds and notification recipients
5. Platform automatically re-screens all investors on schedule

**Alert handling:**

When screening identifies match:

1. Alert appears in **Compliance** &gt; **Alerts**
2. Compliance officer reviews match details
3. Investigates to determine true positive or false positive
4. Actions:
   - **False positive** - Dismiss alert with notes
   - **True positive** - Revoke claims, freeze account, file SAR
   - **Uncertain** - Escalate for enhanced due diligence

![Compliance alerts](/images/screenshots/placeholder.svg) _Screenshot:
Compliance alerts dashboard showing pending alerts with investor name, alert
type, severity, and review button_

### Transaction monitoring

Monitor investor transaction patterns:

1. Navigate to **Compliance** &gt; **Transaction Monitoring**
2. Review flagged transactions:
   - Unusual transaction sizes
   - High frequency trading patterns
   - Transactions exceeding velocity limits
...
- **Geographic risk** - Flag transfers involving high-risk countries
...
3. Choose message type:
   - **Claim expiration reminder** - Auto-send 30 days before expiration
   - **Compliance update** - Notify of new requirements
   - **Transaction notification** - Confirm large transfers
...
For announcements to all investors:

1. **Participants** &gt; **Entities** &gt; **Select All**
2. **Batch Actions** &gt; **Send Announcement**
3. Compose message
4. Send to all or filtered group (e.g., only accredited investors)
...
1. Investor submits address update via dApp
2. Request appears in **Compliance** &gt; **Pending Updates**
3. Review supporting documentation (utility bill, etc.)
4. Approve or reject update
5. If approved, update investor profile and re-issue JURISDICTION claim if
   country changed
...
5. Issue entity-level claims (e.g., QUALIFIED_PURCHASER)
...
3. Issue additional claim (e.g., QUALIFIED_PURCHASER)
4. Investor gains access to additional asset offerings
5. Update investor tier in CRM
...
- Screen against sanctions lists daily (OFAC updates frequently)
...
- Ensure investor not already has OnchainID
...
**Eligibility matrix showing incorrect status**

- Refresh OnchainID claims cache
- Verify compliance module configuration on token
- Check for recently expired claims
- Re-sync identity registry if registry and token state inconsistent
...
- **Compliance architecture** - Deep dive into
...
- **Platform settings** - Configure system-wide compliance in
  [Admin settings](/docs/user-guides/admin-settings)
