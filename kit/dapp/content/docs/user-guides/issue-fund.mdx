...
The contract uses NAV for subscription/redemption pricing calculations.
...
- The contract calculates units: investment amount / NAV per unit
...
**NAV validation:** The platform prevents NAV updates that deviate more than a
...
The platform posts NAV automatically based on oracle data.
...
- The contract calculates proceeds: units × NAV per unit
...
25%), the platform pro-rates all requests or queues excess for the next period.
...
If enabled, the platform automatically:

1. Calculates accrued fees daily: (AUM × fee rate) / 365
2. On collection date, mints new units to your fee wallet
...
- [Corporate actions](/docs/user-guides/corporate-actions) - Execute dividend
  distributions and other fund lifecycle events
- [Manage investors](/docs/user-guides/manage-investors) - Onboard new fund
  investors and verify identities
