...
The page header now includes an **Add admin** button so platform owners can open
the role assignment sheet without scrolling through the permissions table. Use
it when onboarding a new multisig signer—you can still open the same workflow
from individual row menus if you need to adjust a specific wallet.
