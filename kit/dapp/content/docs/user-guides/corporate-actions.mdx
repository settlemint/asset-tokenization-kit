...
Corporate actions are events initiated by asset issuers that affect token
holders, including dividend distributions, coupon payments, token splits, and
governance votes. This guide walks issuers through executing common corporate
actions from planning through settlement.

**Who should read this:** Corporate treasurers, CFOs, investor relations teams,
and asset managers responsible for executing corporate actions on tokenized
assets.

**Time required:** Varies by action type; 30-60 minutes for dividend
...
Common corporate actions supported by the platform:
...
Pay cash or stock dividends to equity holders:

### Plan dividend distribution

Determine dividend parameters:
...
**Automated execution:** Configure automatic execution on payment date to
...
Set up automated bond interest payments:
...
**Accrual calculation:** Platform calculates daily accrual and distributes on
payment dates.
...
**Fractional shares:** Platform handles fractional shares; consider rounding
rules for small holders.
...
Allow automatic distribution reinvestment:
...
- Automate recurring distributions when possible
...
- Check your address holds minimum proposal threshold
- Verify voting enabled on equity token
- Ensure quorum and threshold percentages valid (0-100%)
- Confirm voting period within allowed range

**Vote transaction fails**

- Verify you held shares at proposal creation block
- Check proposal still in voting period
- Ensure you haven't already voted on proposal
- Confirm voting power &gt; 0

**Bond redemption not available**

- Verify bond has reached maturity date
- Check bond matured status set correctly
- Ensure redemption wallet funded
- Confirm you hold bond tokens

**Yield payment failed**

- Check yield wallet has sufficient yield tokens
- Verify yield schedule configured correctly
- Ensure bondholders have verified OnchainIDs
- Review yield module enabled on bond
...
- **Asset-specific actions** - Review asset guides for specialized actions:
  - [Issue a bond](/docs/user-guides/issue-bond) - Bond-specific actions
  - [Issue equity](/docs/user-guides/issue-equity) - Equity governance
  - [Issue a fund](/docs/user-guides/issue-fund) - NAV updates and distributions
- **Investor communications** - Manage shareholder relations via
  [Manage investors](/docs/user-guides/manage-investors)
- **Compliance** - Understand distribution compliance in
