...
The fiat-token bridge enables conversion between traditional fiat currency and
on-chain tokens, facilitating investor onboarding, payment processing, and token
redemption. This guide covers bridge configuration, deposit processing, and
redemption operations.

**Who should read this:** Treasury operations staff, payment processors, banking
integration teams, and customer service representatives handling fiat-crypto
conversions.

**Time required:** Initial setup 2-3 hours; ongoing operations are
...
## Overview of fiat bridge operations
...
Set up fiat-token conversion parameters:
...
Configure receiving bank account for fiat deposits:
...
Set deposit/redemption processing rules:
...
## Step 2: Process fiat deposits

Handle fiat-to-token conversions:
...
Confirm fiat received in bank account:
...
Handle token-to-fiat conversions:

### Receive redemption request

Investor initiates redemption:
...
## Step 4: Monitor bridge operations
...
- Check investor has verified OnchainID
- Verify investor not on blocklist or sanctions list
- Ensure stablecoin contract has sufficient mint capacity
- Confirm platform has SUPPLY_MANAGEMENT_ROLE on token

**Redemption request rejected by platform**

- Verify investor holds sufficient token balance
- Check investor banking details complete
- Ensure token allows redemptions (not paused)
- Verify investor meets minimum redemption amount

**Wire transfer failed or returned**

- Verify banking details accurate (IBAN, SWIFT, account number)
- Check beneficiary name matches bank account
- Ensure bank account accepts international wires
- Contact investor for updated banking information

**Reconciliation shows discrepancy**

- Review unmatched deposits in bank statement
- Check for manual mints not recorded in bridge system
- Verify all redemption payouts marked as complete
- Look for duplicate entries in bridge database
...
- **Stablecoin management** - Review
  [Issue a stablecoin](/docs/user-guides/issue-stablecoin) for reserve
  management
- **Investor onboarding** - Configure
...
- **Compliance** - Understand
  ...
  requirements
- **Treasury operations** - Automate banking integration and reconciliation
  processes
