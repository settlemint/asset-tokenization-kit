...
- The system issues deposit tokens to customer address
...
<Callout type="default" title="For developers">
  ...
  technical details on deposit token implementation and interest accrual
  calculation.
</Callout>
...
**Compliance check:** The system validates customer OnchainID and enforces
per-depositor insurance limits before issuing deposit tokens, automating
compliance that traditionally requires manual review.
...
The system calculates interest automatically based on configured method,
eliminating manual interest calculations and reducing errors.

<Callout type="info" title="Business context">
  Traditional CD interest calculation requires manual computation for each
  certificate, tracking maturity dates in spreadsheets, and coordinating with
  core banking systems to process interest payments. This manual process is
  error-prone and labor-intensive, typically costing $5-15 per certificate per
  payment period. ATK automates the entire interest lifecycle, reducing
  operational costs by 85-95% and eliminating calculation errors.
</Callout>
...
- The system calculates redemption proceeds: principal + interest
...
<Callout type="info" title="Business context">
  Traditional CD maturity processing requires manually identifying maturing
  certificates, calculating interest for each, coordinating with the core
  banking system to credit accounts, and sending maturity notices. This process
  typically takes 2-5 business days and introduces errors in 10-15% of cases.
  ATK automates the entire maturity workflow with zero calculation errors and
  instant settlement.
</Callout>
...
redemptions on maturity date without manual intervention, eliminating the manual
workload of tracking and processing hundreds or thousands of maturing CDs.
...
or opt-out (automatic unless customer declines). The system tracks customer
preferences and executes rollovers automatically, eliminating manual tracking of
renewal elections.

<Callout type="default" title="For developers">
  See [Contract
  Reference](/docs/developer-guides/smart-contracts/contract-reference) for
  technical details on rollover mechanism implementation and maturity date
  updates.
</Callout>
...
- **Configure payments** - Set up
  [Fiat bridge](/docs/user-guides/operations/fiat-bridge) for traditional
  banking integration
- **Manage depositors** - Review
  [Manage investors](/docs/user-guides/operations/manage-investors) for KYC
  workflows
