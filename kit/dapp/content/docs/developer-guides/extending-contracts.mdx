...
### Step 1: Design your asset requirements
...
### Step 2: Create your implementation contract
...
### Step 4: Create factory and proxy contracts
...
`kit/contracts/contracts/assets/bond/ATKBondFactoryImplementation.sol:1-200` and
`kit/contracts/contracts/assets/bond/ATKBondProxy.sol:1-50`.
...
### Step 6: Integrate with the dApp

Once contracts are deployed, integrate with the frontend:

1. **Generate TypeScript types**: Artifacts are auto-generated when you run
   `bun run artifacts` in `kit/contracts`
2. **Access ABIs**: TypeScript definitions appear in
   `kit/dapp/src/lib/contracts/`
3. **Create ORPC routes**: Add API routes in `kit/dapp/src/orpc/routes/` for
...
### Step 1: Understand the module lifecycle
...
### Step 2: Extend the abstract base
...
### Step 3: Test your compliance module

Create tests in
`kit/contracts/test/smart/modules/DailyTransferLimitComplianceModule.t.sol`:
...
### Step 4: Register and use the module
...
`kit/contracts/contracts/addons/README.md:1-493`.
...
Deployment scripts are in `kit/contracts/ignition/modules/` and use Hardhat
Ignition for deterministic deployments.
...
- **Explore compliance modules**: See `kit/contracts/contracts/smart/modules/`
  for built-in examples
- **Review addon system**: Read `kit/contracts/contracts/addons/README.md` for
  airdrop, vault, and settlement patterns
- **Study tests**: Look at `kit/contracts/test/assets/ATKBond.t.sol` for
  comprehensive asset test examples
- **Check deployment**: Review `kit/contracts/ignition/modules/` for production
  deployment patterns
...
[Using the API](/developer-guides/using-api).
