...
<Tabs items={["Full CI Suite", "Unit Tests (DApp)", "Contract Tests", "Subgraph Tests", "E2E Tests"]}>
<Tab value="Full CI Suite">
...
[`kit/dapp/coverage/index.html`](https://github.com/settlemint/asset-tokenization-kit/tree/main/kit/dapp):
...
Creates LCOV report at
[`kit/contracts/coverage/lcov.info`](https://github.com/settlemint/asset-tokenization-kit/tree/main/kit/contracts).
...
[`kit/contracts/test/`](https://github.com/settlemint/asset-tokenization-kit/tree/main/kit/contracts/test)
...
[`kit/e2e/ui-tests/`](https://github.com/settlemint/asset-tokenization-kit/tree/main/kit/e2e/ui-tests)
...
1. **View trace files**:
   ...
   [`kit/e2e/test-results/`](https://github.com/settlemint/asset-tokenization-kit/tree/main/kit/e2e):
...
Run the locale pruning helper whenever you add or rename translation keys. The
utility now loads the TypeScript compiler to analyze every `useTranslation`
call and removes unused strings across all locales, keeping the shipped bundles
small and consistent. If the script reports that `typescript` is missing, run
`bun install` first so the compiler API is available.

```bash
node tools/prune-unused-translations.cjs
```

The script writes the trimmed JSON files back to `kit/dapp/locales`. Include the
resulting diff in your pull request so reviewers can see which namespaces still
power UI copy. If a namespace is no longer referenced, delete the resulting
empty JSON file (`{}`) rather than leaving a placeholder bundle in the tree.
...
- See [Development FAQ](/docs/developer-guides/setup) for common testing issues
- Explore [Code Structure](/docs/developer-guides/architecture) to understand
  test organization
- Consult [API Reference](/docs/developer-guides/api-integration/api-reference)
  for testing API procedures
