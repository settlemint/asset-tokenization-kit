...
### API namespace authentication matrix

The following diagram maps each API namespace to its authentication scope,
...
<Mermaid
  chart={`flowchart TB
    ...
    subgraph TokenScoped["🎫 Token-Scoped"]
        Token(token)
        Actions(actions)
        YieldSchedule(fixedYieldSchedule)
    end
    
    classDef publicStyle fill:#6ba4d4,stroke:#4a7ba8,stroke-width:2px,color:#fff
    classDef authStyle fill:#5fc9bf,stroke:#3a9d96,stroke-width:2px,color:#fff
    classDef tokenStyle fill:#8571d9,stroke:#654bad,stroke-width:2px,color:#fff
    
    class System,ExchangeRates publicStyle
    class User,Account,Settings authStyle
    class Token,Actions,YieldSchedule tokenStyle
`}
/>
...
for observability configuration details.
...
[Subgraph Reference](/docs/architecture/data-indexing/blockchain-indexing).
