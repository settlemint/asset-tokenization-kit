...
Production incidents require structured response workflows to minimize downtime
and ensure proper root cause analysis. The incident response process follows a
clear escalation path from detection through resolution and postmortem.

<Mermaid
  chart={`flowchart TB
    Detection(Alert fires)
    Triage(On-call engineer assesses)
    SeverityCheck{Severity level?}
    Critical(Critical: P0/P1)
    Warning(Warning: P2/P3)
    EscalateCheck{Escalate?}
    Escalate(Escalate to senior team)
    Mitigation(Immediate mitigation)
    Rollback(Option: Rollback deployment)
    Scale(Option: Scale resources)
    Restart(Option: Restart services)
    Communication(Update status page)
    Resolution(Address root cause)
    Verify(Verify resolution)
    Postmortem(Document incident)
    ActionItems(Create action items)
    
    Detection --> Triage
    Triage --> SeverityCheck
    SeverityCheck -->|P0/P1| Critical
    SeverityCheck -->|P2/P3| Warning
    Critical --> EscalateCheck
    Warning --> Mitigation
    EscalateCheck -->|Yes| Escalate
    EscalateCheck -->|No| Mitigation
    Escalate --> Mitigation
    Mitigation --> Rollback
    Mitigation --> Scale
    Mitigation --> Restart
    ...
    class Detection,Verify,ActionItems primary
    class Triage,SeverityCheck,EscalateCheck secondary
    class Critical,Warning,Mitigation,Communication,Postmortem tertiary
    class Escalate,Rollback,Scale,Restart,Resolution quaternary
`}
/>
...
- Root cause requires specialized expertise (smart contracts, blockchain
  infrastructure)
- Incident affects compliance or regulatory obligations
- Multiple components failing simultaneously suggests systemic issue

Escalation contacts maintained in PagerDuty rotation. For P0 incidents, engage
incident commander role to coordinate cross-team response.
...
**Metrics analysis:** Review dashboard metrics for 2 hours before incident.
Compare against baseline to identify anomalies. Look for:
...
Filter to incident time window. Identify first occurrence of errors and trace
backward to triggering event.

**Trace analysis:** Open Tempo and search for failed requests during incident
window. Follow trace spans to identify:
...
Correlate incident timing with deployments, configuration changes, or
...
1. Restarted ERPC deployment to reset upstream health state (14:32 UTC)
2. Verified settlement latency returned to normal &lt;5s (14:35 UTC)
3. Monitored for 30 minutes to confirm stable state

**Corrective actions (with owners and due dates):**

- **Action 1:** Increase ERPC upstream health check timeout from 5s to 30s
  (Owner: DevOps, Due: 2025-01-30)
- **Action 2:** Add alerting for ERPC upstream health check failures (Owner:
  SRE, Due: 2025-02-05)
- **Action 3:** Document ERPC tuning guidelines in runbook (Owner: Tech Writer,
  Due: 2025-02-10)
- **Action 4:** Review all service health check timeouts for appropriate values
  (Owner: Platform Team, Due: 2025-02-15)
...
Store postmortems in shared documentation repository (Git or Confluence).
Reference in future incident response and system design discussions.
