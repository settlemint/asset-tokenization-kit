...
- [Compliance module reference](/docs/developer-guides/smart-contracts/compliance-modules)
  — Smart contract rules that `Token.compliance` enforces

**DALP feature guides**:

- [DvP settlement architecture](/docs/architecture/dvp-settlement) — How atomic
  swaps use `frozenBalance` for escrow
- [Yield distribution system](/docs/architecture/yield-management) — How yield
  schedules scan `TokenBalance` entities
- [Vault custody workflows](/docs/architecture/vault-custody) — Receipt token
  minting and redemption flows

**Operational guides**:

- [Monitoring token metrics](/docs/operations/observability/token-dashboards) —
  Using Grafana dashboards to track supply and transfers
- [Troubleshooting frozen balances](/docs/operations/troubleshooting/frozen-balance-issues)
