...
All database schemas are defined in `kit/dapp/src/lib/db/schemas/` using Drizzle
ORM.

### Authentication schema

**Location**: `kit/dapp/src/lib/db/schemas/auth.ts:1-131`
...
**Location**: `kit/dapp/src/lib/db/schemas/kyc.ts:1-56`
...
**Location**: `kit/dapp/src/lib/db/schemas/exchange-rates.ts:1-304`
...
**Location**: `kit/dapp/src/lib/db/schemas/settings.ts:1-46`
...
**Location**: `kit/subgraph/schema.graphql:1-1500`
...
Managed by Drizzle Kit in `kit/dapp/drizzle.config.ts`.
...
1. Edit `kit/subgraph/schema.graphql`
2. Regenerate types: `bun run codegen`
3. Update mappings in `kit/subgraph/src/`
...
- **Deploy infrastructure**: See
  ...
  production setup
- **Query APIs**: Review
  [API Reference](/developer-guides/api-integration/api-reference) for data
  access patterns
- **Contract interactions**: Check
  [Contract Reference](/developer-guides/smart-contracts/contract-reference) for
  on-chain operations
- **Review code**: Explore implementations in `kit/dapp/src/lib/db/` and
  `kit/subgraph/src/`

For testing strategies that validate data integrity, continue to
[Testing & QA](/developer-guides/quality-assurance/testing-qa).
