...
The address book lives alongside these identity records in the new `contact`
table. Each row links a user (`user_id`) to a named wallet destination, giving
portfolio workflows a reusable set of counterparties. The ORPC surface exposes
this data via `orpc.contacts.list` for paginated browsing, `orpc.contacts.search`
for autocomplete, `orpc.contacts.upsert` for creation or edits, and
`orpc.contacts.delete` for removals. All routes enforce wallet ownership and
apply case-insensitive matches on both name and wallet to keep lookups fast.
