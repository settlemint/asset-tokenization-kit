...
For rapid prototyping in local environments, `bun run db:push` applies schema
changes directly without generating migration files, but this should never be
used in production environments. The observability stack monitors migration
status and database schema version to detect drift between deployed
environments.
...
[Observability guide](/docs/developer-guides/observability) and
[Architecture observability section](/docs/architecture/observability).
...
[Observability guide](/docs/developer-guides/observability).
