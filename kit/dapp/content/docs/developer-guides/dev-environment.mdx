...
**Note:** Re-run `bun run artifacts` whenever you modify:

- Smart contracts in `kit/contracts/contracts/`
- Database schemas in `kit/dapp/src/lib/db/schemas/`
- Subgraph mappings in `kit/subgraph/src/`
...
**Optional:** Create a `.env.local` file in the project root to override
defaults:
...
POSTGRES_PORT=5432
HASURA_PORT=8080
GRAPH_NODE_PORT=8000
MINIO_CONSOLE_PORT=9001
BLOCKSCOUT_FRONTEND_PORT=4001
```
...
**Troubleshooting:** If services fail to start:

- Check Docker has sufficient memory allocated (8+ GB in Docker Desktop
  settings)
- Ensure ports are not already in use: `lsof -i :8545,5432,8080`
...
- **Deploy your first token**: Follow the
  [Getting started](/user-guides/getting-started) tutorial
- **Explore the codebase**: See
  [Code structure](/developer-guides/code-structure)
- **Extend smart contracts**: Read
  [Extending contracts](/developer-guides/extending-contracts)
