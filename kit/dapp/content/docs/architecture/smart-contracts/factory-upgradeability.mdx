...
graph TB
    ...
    style Addons fill:#6ba4d4,stroke:#4a7ba8,stroke-width:2px,color:#fff
    style Assets fill:#8571d9,stroke:#654bad,stroke-width:2px,color:#fff
    style System fill:#5fc9bf,stroke:#3a9d96,stroke-width:2px,color:#fff
...
Tokenized financial instruments built on the SMART Protocol using ATK's
...
External contracts and libraries that the ATK Protocol depends on, including
OpenZeppelin upgradeable contracts, ERC standards implementations, and other
audited dependencies.

### Role-based access control

The ATK system uses a dual-category role system that separates human operator
permissions from automated contract permissions. This structure ensures clear
separation between administrative actions and system automation.
...
**SYSTEM_MANAGER_ROLE** - Bootstrap system, manage upgrades, register factories
and modules
...
See
[Asset Contracts - Role definitions](/docs/architecture/smart-contracts/asset-contracts#role-definitions)
for detailed role capabilities on individual tokens.
...
The factory and proxy architecture provides the foundation for asset
tokenization:
...
tokenization solutions while maintaining the security and auditability required
