...
indexing. This sequence executes atomically—either all steps succeed, or the
deployment reverts.
...
The [subgraph](/docs/architecture/backend-indexing/subgraph-architecture)
indexes these events, building a complete upgrade history for every contract in
the system. Auditors query this history to verify upgrade frequency, review
upgrade authorization, and ensure upgrade procedures followed governance rules.
...
to new contract addresses—a complex, expensive, and risky operation. The upgrade
...
[observability dashboard](/docs/architecture/backend-indexing/observability-monitoring)
...
Upgradeable proxies enable the system to evolve—fixing bugs, adding features,
and adapting to regulatory changes—without disrupting deployed tokens or
...
infrastructure for regulated financial instruments—secure enough for
