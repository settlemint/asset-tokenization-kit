...
description: The Asset Tokenization Kit frontend is built as a modern, type-safe
  single-page application using TanStack's ecosystem. It delivers a responsive,
  accessible user experience for managing tokenized assets while maintaining
  strict type safety from backend to browser.
...
[`src/components/ui/`](https://github.com/settlemint/asset-tokenization-kit/tree/main/kit/dapp/src/components/ui)
...
[`src/routes/`](https://github.com/settlemint/asset-tokenization-kit/tree/main/kit/dapp/src/routes)
...
flowchart TB
    ...
    style Root fill:#6ba4d4,stroke:#4a7ba8,color:#fff
    style Private fill:#8571d9,stroke:#654bad,color:#fff
    style Onboarded fill:#5fc9bf,stroke:#3a9d96,color:#fff
    style Sidebar fill:#5fc9bf,stroke:#3a9d96,color:#fff
    style Pages fill:#5fc9bf,stroke:#3a9d96,color:#fff
...
flowchart LR
    ...
    style UI fill:#8571d9,stroke:#654bad,color:#fff
    style Query fill:#5fc9bf,stroke:#3a9d96,color:#fff
    style ORPC fill:#6ba4d4,stroke:#4a7ba8,color:#fff
...
**1. ORPC API**
([`src/orpc/`](https://github.com/settlemint/asset-tokenization-kit/tree/main/kit/dapp/src/orpc)) -
...
[`src/routes/_private/_onboarded/_sidebar/addon-designer.tsx`](https://github.com/settlemint/asset-tokenization-kit/blob/main/kit/dapp/src/routes/_private/_onboarded/_sidebar/addon-designer.tsx):
...
flowchart TB
    ...
    style Details fill:#6ba4d4,stroke:#4a7ba8,stroke-width:2px,color:#fff
    ...
    style Deploy fill:#8571d9,stroke:#654bad,stroke-width:2px,color:#fff
    style Complete fill:#5fc9bf,stroke:#3a9d96,stroke-width:2px,color:#fff

    style Start fill:#6ba4d4,stroke:#4a7ba8,color:#fff
    style Details fill:#6ba4d4,stroke:#4a7ba8,color:#fff
    style Compliance fill:#b661d9,stroke:#8a3fb3,color:#fff
    style Extensions fill:#8571d9,stroke:#654bad,color:#fff
    style Review fill:#8571d9,stroke:#654bad,color:#fff
    style Deploy fill:#b661d9,stroke:#8a3fb3,color:#fff
    style Complete fill:#5fc9bf,stroke:#3a9d96,color:#fff
...
- **Verifications** - Card-based detail view with an inline Add Claim button,
  opening the issue-claim sheet above the claims table for quick issuance
...
The frontend connects to backend services exclusively through the ORPC API
layer:

- **ORPC API** (`/api/rpc`) - All business logic, database queries, and
  blockchain data access
- **SettleMint Portal** - Blockchain transaction submission (wallet-initiated)
- **MinIO** - Document storage access (via signed URLs from ORPC)

The ORPC API layer internally coordinates with TheGraph subgraph, PostgreSQL,
and other data sources. This architecture ensures consistent authentication,
validation, and error handling across all data access patterns.
