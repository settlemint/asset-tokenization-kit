...
description:
  The Asset Tokenization Kit implements a unified identity and compliance
  control plane that spans smart contracts, off-chain KYC workflows, and API
  enforcement layers. This architecture ensures that compliance rules defined
  once propagate consistently across all platform operations.
pageTitle: Identity & compliance control plane - End-to-end verification architecture
...
- **Registration states**: The dApp surfaces `Registered` and
  `Pending registration` badges to mirror on-chain registration progress for
  each identity
...
<Mermaid
  chart={`flowchart TB
    ...
    subgraph "Smart Contract Layer"
        IdentityRegistry(Identity Registry<br/>Address → Identity Mapping)
    ...
    style User fill:#8571d9,stroke:#654bad,stroke-width:2px,color:#fff
    style Compliance fill:#b661d9,stroke:#8a3fb3,stroke-width:2px,color:#fff
    style IdentityRegistry fill:#5fc9bf,stroke:#3a9d96,stroke-width:2px,color:#fff
    style Token fill:#6ba4d4,stroke:#4a7ba8,stroke-width:2px,color:#fff
`}
/>
...
<Mermaid
  chart={`flowchart LR
    ...
    style J fill:#b661d9,stroke:#8a3fb3,stroke-width:2px,color:#fff
    style K fill:#5fc9bf,stroke:#3a9d96,stroke-width:2px,color:#fff
    style L fill:#b661d9,stroke:#8a3fb3,stroke-width:2px,color:#fff
`}
/>
...
<Mermaid
  chart={`flowchart LR
    ...
    style Adapter fill:#8571d9,stroke:#654bad,stroke-width:2px,color:#fff
    style KYCService fill:#5fc9bf,stroke:#3a9d96,stroke-width:2px,color:#fff
    style ClaimIssuer fill:#6ba4d4,stroke:#4a7ba8,stroke-width:2px,color:#fff
`}
/>
...
Identity operators without a connected wallet can still review entity
identities; the dApp skips privileged token lookups when authentication falls
back to identity permissions or when the identity record lacks an associated
account.
...
- [Database model](../data-indexing/database-model) - KYC application and audit
  log schemas
- [Compliance certification](./compliance-certification) - Jurisdiction-specific
