...
description:
  ...
  coordinates between frontend, database, and blockchain layers. Built on ORPC
  for type-safe procedures and Better Auth for authentication, it provides the
  middle tier that transforms user intentions into validated, auditable
  operations.
...
ensures `totalSupply` is a positive integer—all at compile time. If backend
...
**Corporate actions** manage scheduled events. Yield schedules calculate
entitlement amounts based on token balances at snapshot block for claim-based
distribution. Dividend claims coordinate with payment gateway to release cash.
Redemption processes burn tokens and trigger asset delivery workflows.
