...
<Mermaid
  chart={`flowchart TB
    ...
    style Procedures fill:#10b981,stroke:#047857,stroke-width:2px,color:#fff
    style TheGraphClient fill:#0ea5e9,stroke:#0369a1,stroke-width:2px,color:#fff
    style Subgraph fill:#d97706,stroke:#92400e,stroke-width:2px,color:#fff
`}
/>
...
<Mermaid
  chart={`flowchart TB
    ...
    style TokenStats fill:#0ea5e9,stroke:#0369a1,stroke-width:2px,color:#fff
`}
/>
...
Query from components with TanStack Query:

```typescript
import { useQuery } from "@tanstack/react-query";
import { GetTokenDocument } from "@/lib/subgraph/queries/token.generated";
import { subgraphClient } from "@/lib/subgraph/client";
...
export function useToken(tokenId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const result = await subgraphClient.query(GetTokenDocument, { tokenId });
