...
import { DvPSwapStatusType, EventName } from "../utils/enums";
...
export function handleDvPSwapCreated(event: DvPSwapCreated): void {
  ...
  const dvpSwapEntity = fetchDvPSwapEntity(
    event.params.dvpSwapId,
    event.address,
  ...
  dvpSwapCreatedEvent.amountToReceive = toDecimals(
  ...
  // Track TVL
  dvpSwap.totalValueLockedExact = dvpSwap.totalValueLockedExact.plus(
    event.params.amountToSend
  );
...
export function handleDvPSwapStatusChanged(event: DvPSwapStatusChanged): void {
  ...
  log.info("DvPSwap - Status changed: dvpSwapId={}, status={}", [
    event.params.dvpSwapId.toHexString(),
    statusString,
  ]);
}
...
export function handleDvPSwapClaimed(event: DvPSwapClaimed): void {
  ...
  // Update TVL
  dvpSwap.totalValueLockedExact = dvpSwap.totalValueLockedExact.minus(
    dvpSwapEntity.amountToSendExact
  );
  dvpSwap.totalValueLocked = toDecimals(dvpSwap.totalValueLockedExact, 18);
  ...
  log.info("DvPSwap - DvPSwap claimed: dvpSwapId={}, receiver={}, secret={}", [
    event.params.dvpSwapId.toHexString(),
    receiver.id.toHexString(),
...
export function handleDvPSwapRefunded(event: DvPSwapRefunded): void {
  ...
  log.info("DvPSwap - DvPSwap refunded: dvpSwapId={}, refundedTo={}", [
    event.params.dvpSwapId.toHexString(),
