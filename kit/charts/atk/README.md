...
| observability.prometheus-node-exporter | object | `{"image":{"registry":"quay.io"},"kubeRBACProxy":{"image":{"registry":"quay.io"}}}` | Prometheus Node Exporter for host metrics |
...
| observability.victoria-metrics-single | object | `{"global":{"image":{"registry":"docker.io"}},"server":{"image":{"registry":"docker.io"},"persistentVolume":{"size":"10Gi","storageClass":""},"resources":{}}}` | VictoriaMetrics time-series database for metrics storage |
| observability.victoria-metrics-single.global | object | `{"image":{"registry":"docker.io"}}` | Global VictoriaMetrics image configuration |
...
| portal | object | `{"enabled":true,"image":{"registry":"ghcr.io"},"ingress":{"enabled":true,"hostname":"portal.k8s.orb.local","ingressClassName":"atk-nginx"},"initContainer":{"tcpCheck":{"image":{"repository":"ghcr.io/settlemint/btp-waitforit"}}},"tests":{"image":{"registry":"docker.io"}}}` | Portal identity and access management configuration |
...
| support.ingress-nginx.controller.opentelemetry | object | `{"image":{"registry":"registry.k8s.io"}}` | OpenTelemetry sidecar image configuration |
...
| support.ingress-nginx.controller.patch | object | `{"image":{"registry":"registry.k8s.io"}}` | Patch job image configuration |
...
| support.ingress-nginx.global | object | `{"image":{"registry":"registry.k8s.io"}}` | Global ingress-nginx image configuration |
