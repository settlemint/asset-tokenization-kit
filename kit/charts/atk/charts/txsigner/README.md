...
| autoscaling | object | `{"builtInMetrics":[{"resource":{"name":"cpu","target":{"averageUtilization":80,"type":"Utilization"}},"type":"Resource"},{"resource":{"name":"memory","target":{"averageUtilization":80,"type":"Utilization"}},"type":"Resource"}],"customMetrics":[],"enabled":false,"maxReplicas":3,"minReplicas":1}` | Autoscaling configuration for TxSigner (object) |
...
| autoscaling.builtInMetrics[0] | object | `{"resource":{"name":"cpu","target":{"averageUtilization":80,"type":"Utilization"}},"type":"Resource"}` | CPU resource metric configuration |
| autoscaling.builtInMetrics[0].resource | object | `{"name":"cpu","target":{"averageUtilization":80,"type":"Utilization"}}` | Resource metric details |
| autoscaling.builtInMetrics[0].resource.name | string | `"cpu"` | Resource name (cpu or memory) |
| autoscaling.builtInMetrics[0].resource.target | object | `{"averageUtilization":80,"type":"Utilization"}` | Target metric threshold |
...
| autoscaling.builtInMetrics[1] | object | `{"resource":{"name":"memory","target":{"averageUtilization":80,"type":"Utilization"}},"type":"Resource"}` | Memory resource metric configuration |
| autoscaling.builtInMetrics[1].resource | object | `{"name":"memory","target":{"averageUtilization":80,"type":"Utilization"}}` | Resource metric details |
| autoscaling.builtInMetrics[1].resource.name | string | `"memory"` | Resource name (cpu or memory) |
| autoscaling.builtInMetrics[1].resource.target | object | `{"averageUtilization":80,"type":"Utilization"}` | Target metric threshold |
...
| config.queue | object | `{"maxSize":1000,"processingInterval":1000}` | Transaction queue configuration (object) |
...
| config.rateLimit | object | `{"enabled":true,"maxRequestsPerHour":1000,"maxRequestsPerMinute":60}` | Rate limiting configuration (object) |
...
| extraVolumeMounts[0] | object | `{"mountPath":"/signer/.cache","name":"tx-signer-cache"}` | Cache volume mount configuration |
...
| extraVolumes[0] | object | `{"emptyDir":{},"name":"tx-signer-cache"}` | Cache volume configuration |
...
| image | object | `{"digest":"","pullPolicy":"IfNotPresent","pullSecrets":[],"registry":"harbor.settlemint.com/ghcr.io","repository":"settlemint/btp-signer","tag":"7.15.13"}` | TxSigner image |
...
| initContainer.tcpCheck.dependencies | list | `[{"endpoint":"{{ include \"atk.postgresql.endpoint\" (dict \"context\" $ \"chartKey\" \"txsigner\") }}","name":"postgresql"}]` | List of dependencies to check before starting |
| initContainer.tcpCheck.dependencies[0] | object | `{"endpoint":"{{ include \"atk.postgresql.endpoint\" (dict \"context\" $ \"chartKey\" \"txsigner\") }}","name":"postgresql"}` | PostgreSQL dependency configuration |
...
| initContainer.tcpCheck.image | object | `{"pullPolicy":"IfNotPresent","repository":"harbor.settlemint.com/ghcr.io/settlemint/btp-waitforit","tag":"v7.7.10"}` | Init container image configuration |
...
| initContainer.tcpCheck.resources | object | `{"limits":{"cpu":"100m","memory":"64Mi"},"requests":{"cpu":"10m","memory":"32Mi"}}` | Init container resource requests and limits |
...
| networkPolicy.extraEgress[0] | object | `{"ports":[{"port":53,"protocol":"UDP"}],"to":[{"namespaceSelector":{},"podSelector":{"matchLabels":{"k8s-app":"kube-dns"}}}]}` | Allow DNS resolution |
| networkPolicy.extraEgress[0].ports | list | `[{"port":53,"protocol":"UDP"}]` | Allowed ports for DNS |
| networkPolicy.extraEgress[0].ports[0] | object | `{"port":53,"protocol":"UDP"}` | DNS UDP port configuration |
| networkPolicy.extraEgress[0].ports[0].port | int | `53` | DNS port |
| networkPolicy.extraEgress[0].to[0] | object | `{"namespaceSelector":{},"podSelector":{"matchLabels":{"k8s-app":"kube-dns"}}}` | Allow egress to kube-dns pods |
| networkPolicy.extraEgress[0].to[0].podSelector | object | `{"matchLabels":{"k8s-app":"kube-dns"}}` | Match kube-dns pods |
| networkPolicy.extraEgress[0].to[0].podSelector.matchLabels | object | `{"k8s-app":"kube-dns"}` | Match labels for kube-dns |
| networkPolicy.extraEgress[0].to[0].podSelector.matchLabels.k8s-app | string | `"kube-dns"` | Application name for kube-dns |
| networkPolicy.extraEgress[1] | object | `{"ports":[{"port":8545,"protocol":"TCP"}],"to":[{"podSelector":{"matchLabels":{"app.kubernetes.io/name":"besu-statefulset"}}}]}` | Allow access to Besu nodes |
| networkPolicy.extraEgress[1].ports | list | `[{"port":8545,"protocol":"TCP"}]` | Allowed ports for Besu RPC |
...
| networkPolicy.extraEgress[1].to[0] | object | `{"podSelector":{"matchLabels":{"app.kubernetes.io/name":"besu-statefulset"}}}` | Allow egress to Besu statefulset pods |
| networkPolicy.extraEgress[1].to[0].podSelector.matchLabels | object | `{"app.kubernetes.io/name":"besu-statefulset"}` | Match labels for Besu |
...
| networkPolicy.extraEgress[2] | object | `{"ports":[{"port":443,"protocol":"TCP"}],"to":[{"namespaceSelector":{}}]}` | Allow access to external HTTPS APIs (for signing services) |
| networkPolicy.extraEgress[2].ports | list | `[{"port":443,"protocol":"TCP"}]` | Allowed ports for HTTPS |
| networkPolicy.extraEgress[2].ports[0] | object | `{"port":443,"protocol":"TCP"}` | HTTPS port configuration |
| networkPolicy.extraEgress[2].ports[0].port | int | `443` | HTTPS port |
| networkPolicy.extraEgress[2].to[0] | object | `{"namespaceSelector":{}}` | Allow egress to any namespace |
...
| networkPolicy.extraIngress[0] | object | `{"from":[{"podSelector":{"matchLabels":{"app.kubernetes.io/name":"dapp"}}},{"podSelector":{"matchLabels":{"app.kubernetes.io/name":"portal"}}},{"podSelector":{"matchLabels":{"app.kubernetes.io/name":"ingress-nginx"}}},{"podSelector":{}}],"ports":[{"port":3000,"protocol":"TCP"},{"port":3001,"protocol":"TCP"}]}` | Allow ingress from dapp, portal, ingress-nginx, and same namespace |
| networkPolicy.extraIngress[0].from[0] | object | `{"podSelector":{"matchLabels":{"app.kubernetes.io/name":"dapp"}}}` | Allow from dapp pods |
| networkPolicy.extraIngress[0].from[0].podSelector.matchLabels | object | `{"app.kubernetes.io/name":"dapp"}` | Match labels for dapp |
| networkPolicy.extraIngress[0].from[0].podSelector.matchLabels."app.kubernetes.io/name" | string | `"dapp"` | Application name for dapp |
| networkPolicy.extraIngress[0].from[1] | object | `{"podSelector":{"matchLabels":{"app.kubernetes.io/name":"portal"}}}` | Allow from portal pods |
| networkPolicy.extraIngress[0].from[1].podSelector.matchLabels | object | `{"app.kubernetes.io/name":"portal"}` | Match labels for portal |
...
| networkPolicy.extraIngress[0].from[2] | object | `{"podSelector":{"matchLabels":{"app.kubernetes.io/name":"ingress-nginx"}}}` | Allow from ingress-nginx pods |
| networkPolicy.extraIngress[0].from[2].podSelector.matchLabels | object | `{"app.kubernetes.io/name":"ingress-nginx"}` | Match labels for ingress-nginx |
...
| networkPolicy.extraIngress[0].from[3] | object | `{"podSelector":{}}` | Allow from all pods in same namespace |
| networkPolicy.extraIngress[0].ports | list | `[{"port":3000,"protocol":"TCP"},{"port":3001,"protocol":"TCP"}]` | Allowed ports for ingress |
| networkPolicy.extraIngress[0].ports[0] | object | `{"port":3000,"protocol":"TCP"}` | HTTP port configuration |
| networkPolicy.extraIngress[0].ports[0].port | int | `3000` | HTTP port |
| networkPolicy.extraIngress[0].ports[1] | object | `{"port":3001,"protocol":"TCP"}` | Metrics port configuration |
...
| tests | object | `{"image":{"pullPolicy":"IfNotPresent","registry":"harbor.settlemint.com/docker.io","repository":"busybox","tag":"1.37.0"}}` | Test parameters (object) |
