...
helm.sh/chart: {{ include "txsigner.chart" $context }}
...
{{- if semverCompare "<1.14-0" (include "txsigner.capabilities.kubeVersion" .) -}}
...
{{- if semverCompare "<1.14-0" (include "txsigner.capabilities.kubeVersion" .) -}}
{{- print "extensions/v1beta1" -}}
{{- else if semverCompare "<1.19-0" (include "txsigner.capabilities.kubeVersion" .) -}}
...
{{- $context := .context | default . -}}
{{- if semverCompare "<1.23-0" (include "txsigner.capabilities.kubeVersion" $context) -}}
...
{{- if semverCompare "<1.21-0" (include "txsigner.capabilities.kubeVersion" .) -}}
...
preferredDuringSchedulingIgnoredDuringExecution:
  - weight: 1
    podAffinityTerm:
      labelSelector:
        matchLabels: {{- (include "txsigner.selectorLabels" $context) | nindent 10 }}
          {{- with $customLabels }}
          {{- toYaml . | nindent 10 }}
          {{- end }}
...
requiredDuringSchedulingIgnoredDuringExecution:
  - labelSelector:
      matchLabels: {{- (include "txsigner.selectorLabels" $context) | nindent 8 }}
        {{- with $customLabels }}
        {{- toYaml . | nindent 8 }}
        {{- end }}
    topologyKey: kubernetes.io/hostname
...
{{- $context := .context -}}
{{- if semverCompare "<1.19-0" (include "txsigner.capabilities.kubeVersion" $context) }}
serviceName: {{ $serviceName }}
servicePort: {{ $servicePort }}
{{- else }}
service:
...
{{- if semverCompare "<1.18-0" (include "txsigner.capabilities.kubeVersion" .) -}}
{{- print "false" -}}
{{- else -}}
{{- print "true" -}}
{{- end -}}
