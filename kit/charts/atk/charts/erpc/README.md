...
| autoscaling | object | `{"builtInMetrics":[{"resource":{"name":"cpu","target":{"averageUtilization":80,"type":"Utilization"}},"type":"Resource"},{"resource":{"name":"memory","target":{"averageUtilization":80,"type":"Utilization"}},"type":"Resource"}],"customMetrics":[],"enabled":false,"maxReplicas":3,"minReplicas":1}` | Autoscaling configuration for eRPC (object) |
| autoscaling.builtInMetrics | list | `[{"resource":{"name":"cpu","target":{"averageUtilization":80,"type":"Utilization"}},"type":"Resource"},{"resource":{"name":"memory","target":{"averageUtilization":80,"type":"Utilization"}},"type":"Resource"}]` | Built-in metrics configuration (list) |
| autoscaling.customMetrics | list | `[]` | Custom metrics configuration (list) |
| autoscaling.enabled | bool | `false` | Enable autoscaling for eRPC (bool) |
| autoscaling.maxReplicas | int | `3` | Maximum number of eRPC replicas (int) |
| autoscaling.minReplicas | int | `1` | Minimum number of eRPC replicas (int) |
...
| config | object | `{"database":{"evmJsonRpcCache":{"compression":{"algorithm":"zstd","enabled":true,"threshold":1024,"zstdLevel":"default"},"connectors":[{"driver":"redis","id":"redis-cache","redis":{"uri":"{{ include \"erpc.redis.uriFor\" (dict \"context\" $ \"dbKey\" \"cacheDb\" \"queryKey\" \"cacheQuery\") }}"}}],"policies":[{"connector":"redis-cache","finality":"finalized","method":"*","network":"*","ttl":0},{"connector":"redis-cache","finality":"unfinalized","method":"*","network":"*","ttl":"5s"},{"connector":"redis-cache","finality":"realtime","method":"*","network":"*","ttl":"5s"}]},"sharedState":{"clusterKey":"atk-erpc-shared","connector":{"driver":"redis","redis":{"uri":"{{ include \"erpc.redis.uriFor\" (dict \"context\" $ \"dbKey\" \"sharedStateDb\" \"queryKey\" \"sharedStateQuery\") }}"}},"fallbackTimeout":"5s","lockTtl":"30s"}},"logLevel":"info","metrics":{"enabled":true,"hostV4":"0.0.0.0","port":4001},"projects":[],"server":{"httpHostV4":"0.0.0.0","httpPort":4000,"waitAfterShutdown":"30s","waitBeforeShutdown":"30s"}}` | eRPC configuration (object) |
...
| config.logLevel | string | `"info"` | Log level for eRPC (string) |
| config.metrics | object | `{"enabled":true,"hostV4":"0.0.0.0","port":4001}` | Metrics endpoint configuration (list) |
| config.projects | list | `[]` | Array of project configurations (will be overridden by parent chart) |
| config.server | object | `{"httpHostV4":"0.0.0.0","httpPort":4000,"waitAfterShutdown":"30s","waitBeforeShutdown":"30s"}` | Server configuration (object) |
| configMountPath | string | `"/erpc.yaml"` | Path where the rendered configuration file will be mounted (object) |
| containerSecurityContext | object | `{}` | Container Security Context configuration (overrides global.securityContexts.container) |
| envSecret | object | `{"defaultMode":420,"enabled":false,"mountPath":"/.env","name":"","subPath":""}` | .env Secret mount configuration (object) |
| envSecret.defaultMode | int | `420` | File mode applied to the mounted Secret (decimal 420 = 0644) |
| envSecret.enabled | bool | `false` | Enable mounting a Secret containing a .env file (bool) |
| envSecret.mountPath | string | `"/.env"` | Path where the .env file will be mounted (string) |
...
| extraEnvVars | list | `[]` | Array with extra environment variables to add to eRPC nodes (list) |
| extraEnvVarsCM | string | `""` | Name of existing ConfigMap containing extra env vars for eRPC nodes (list) |
| extraEnvVarsSecret | string | `""` | Name of existing Secret containing extra env vars for eRPC nodes (list) |
| extraInitContainers | list | `[]` | Additional init containers appended verbatim to the workload spec (list) |
...
| fullnameOverride | string | `"erpc"` | String to fully override common.names.fullname (string) |
| global | object | `{"datastores":{},"imagePullSecrets":[],"imageRegistry":"","securityContexts":{},"storageClass":""}` | Global Docker image registry (object) |
...
| image | object | `{"digest":"","pullPolicy":"IfNotPresent","pullSecrets":[],"registry":"ghcr.io","repository":"erpc/erpc","tag":"0.0.56"}` | eRPC image (object) |
| image.digest | string | `""` | eRPC image digest in the way sha256:aa.... Please note this parameter, if set, will override the tag (object) |
| image.pullPolicy | string | `"IfNotPresent"` | eRPC image pull policy (object) |
| image.pullSecrets | list | `[]` | eRPC image pull secrets (list) |
| image.registry | string | `"ghcr.io"` | eRPC image registry (object) |
| image.repository | string | `"erpc/erpc"` | eRPC image repository (object) |
| image.tag | string | `"0.0.56"` | eRPC image tag (immutable tags are recommended) |
| ingress | object | `{"annotations":{},"apiVersion":"","enabled":true,"extraHosts":[],"extraPaths":[],"extraRules":[],"extraTls":[],"hostname":"besu.k8s.orb.local","ingressClassName":"atk-nginx","path":"/","pathType":"ImplementationSpecific","secrets":[],"selfSigned":false,"tls":false}` | Ingress parameters (object) |
...
| ingress.enabled | bool | `true` | Enable ingress record generation for eRPC (bool) |
...
| ingress.hostname | string | `"besu.k8s.orb.local"` | Default host for the ingress record (object) |
...
| livenessProbe | object | `{"enabled":true,"failureThreshold":3,"httpGet":{"path":"/healthcheck","port":"http"},"initialDelaySeconds":10,"periodSeconds":10,"successThreshold":1,"timeoutSeconds":5}` | Configure eRPC containers' liveness probe (object) |
| livenessProbe.enabled | bool | `true` | Enable livenessProbe on eRPC containers (bool) |
| livenessProbe.failureThreshold | int | `3` | Failure threshold for livenessProbe (int) |
| livenessProbe.httpGet | object | `{"path":"/healthcheck","port":"http"}` | HTTP get parameters for livenessProbe (object) |
| livenessProbe.httpGet.path | string | `"/healthcheck"` | Path for httpGet livenessProbe (object) |
| livenessProbe.httpGet.port | string | `"http"` | Port for httpGet livenessProbe (int) |
| livenessProbe.initialDelaySeconds | int | `10` | Initial delay seconds for livenessProbe (int) |
...
| networkPolicy.ingressRules.accessOnlyFrom.enabled | bool | `false` | Enable ingress rule that makes eRPC only accessible from a particular origin. (bool) |
...
| podAnnotations | object | `{"prometheus.io/path":"/metrics","prometheus.io/port":"4001","prometheus.io/scrape":"true"}` | Annotations for eRPC pods (object) |
| podAnnotations."prometheus.io/path" | string | `"/metrics"` | Prometheus metrics path (list) |
| podAnnotations."prometheus.io/port" | string | `"4001"` | Prometheus metrics port (int) |
...
| podLabels | object | `{}` | Extra labels for eRPC pods (list) |
| podSecurityContext | object | `{}` | Pod Security Context configuration (overrides global.securityContexts.pod) |
| priorityClassName | string | `""` | eRPC pods' priority class name (string) |
| readinessProbe | object | `{"enabled":true,"failureThreshold":3,"httpGet":{"path":"/healthcheck","port":"http"},"initialDelaySeconds":5,"periodSeconds":10,"successThreshold":1,"timeoutSeconds":5}` | Configure eRPC containers' readiness probe (object) |
| readinessProbe.enabled | bool | `true` | Enable readinessProbe on eRPC containers (bool) |
| readinessProbe.failureThreshold | int | `3` | Failure threshold for readinessProbe (int) |
| readinessProbe.httpGet | object | `{"path":"/healthcheck","port":"http"}` | HTTP get parameters for readinessProbe (object) |
| readinessProbe.httpGet.path | string | `"/healthcheck"` | Path for httpGet readinessProbe (object) |
| readinessProbe.httpGet.port | string | `"http"` | Port for httpGet readinessProbe (int) |
| readinessProbe.initialDelaySeconds | int | `5` | Initial delay seconds for readinessProbe (int) |
| readinessProbe.periodSeconds | int | `10` | Period seconds for readinessProbe (int) |
...
| replicaCount | int | `1` | Number of eRPC replicas to deploy (int) |
| resources | object | `{}` | eRPC containers resource requests and limits (object) |
| runtime | object | `{"gc":{"enabled":true,"gogc":30,"gomemlimitOverride":"","gomemlimitRatio":0.85}}` | Runtime tuning (string) |
| runtime.gc.enabled | bool | `true` | Enable automatic Go GC tuning based on container limits (bool) |
| runtime.gc.gogc | int | `30` | Triggers garbage collection when heap grows by this percentage (string) |
...
| service | object | `{"annotations":{},"clusterIP":"","externalTrafficPolicy":"Cluster","extraPorts":[],"loadBalancerIP":"","loadBalancerSourceRanges":[],"metricsNodePort":"","metricsPort":4001,"nodePort":"","port":4000,"sessionAffinity":"None","sessionAffinityConfig":{},"type":"ClusterIP"}` | Service parameters (object) |
| service.annotations | object | `{}` | Additional custom annotations for eRPC service (list) |
| service.clusterIP | string | `""` | eRPC service Cluster IP (object) |
| service.externalTrafficPolicy | string | `"Cluster"` | eRPC service external traffic policy (object) |
| service.extraPorts | list | `[]` | Extra ports to expose in the eRPC service (normally used with the `sidecar` value) |
| service.loadBalancerIP | string | `""` | eRPC service Load Balancer IP (object) |
| service.loadBalancerSourceRanges | list | `[]` | eRPC service Load Balancer sources (list) |
| service.metricsNodePort | string | `""` | Node port for metrics (int) |
| service.metricsPort | int | `4001` | eRPC service metrics port (int) |
| service.nodePort | string | `""` | Node port for HTTP (int) |
| service.port | int | `4000` | eRPC service HTTP port (int) |
...
| service.type | string | `"ClusterIP"` | eRPC service type (object) |
| serviceAccount | object | `{"annotations":{},"automountServiceAccountToken":false,"create":true,"labels":{},"name":""}` | Service account for eRPC pods (int) |
...
| startupProbe | object | `{"enabled":false,"failureThreshold":10,"initialDelaySeconds":30,"periodSeconds":10,"successThreshold":1,"timeoutSeconds":5}` | Configure eRPC containers' startup probe (object) |
| startupProbe.enabled | bool | `false` | Enable startupProbe on eRPC containers (bool) |
...
| updateStrategy | object | `{"rollingUpdate":{},"type":"RollingUpdate"}` | Update strategy configuration for eRPC deployment (object) |
| updateStrategy.rollingUpdate | object | `{}` | eRPC deployment rolling update configuration parameters (object) |
| updateStrategy.type | string | `"RollingUpdate"` | eRPC deployment strategy type (object) |
| workingDir | string | `"/"` | Working directory for the eRPC container (string) |
