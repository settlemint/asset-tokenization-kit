...
async function hasDependencies(chartYamlPath: string): Promise<boolean> {
  ...
  return (
    /^\s*dependencies:\s*$/im.test(content) || content.includes('dependencies:')
  );
}
 ...
 */
async function getChartsWithDependencies(
  projectDir: string
): Promise<string[]> {
 ...
 */
async function updateHelmDependencies(
  chartDir: string,
  maxRetries = 3
): Promise<void> {
  const chartName = chartDir.split('/').pop() || 'unknown';
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (attempt < maxRetries) {
        logger.warn(
          `Failed to update dependencies for ${chartName} (attempt ${attempt}/${maxRetries}): ${error}`
        );
        logger.info(`Retrying in ${attempt * 2} seconds...`);
        await Bun.sleep(attempt * 2000); // Exponential backoff: 2s, 4s, 6s
      } else {
        logger.error(
          `Failed to update dependencies for ${chartName} after ${maxRetries} attempts: ${error}`
        );
      }
    }
  }
